...
<?php
// add_products.php - Versi dengan CSS terpisah

include 'config/db_connect.php';

// Fixed data kategori & subkategori
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $condition = $_POST['condition'] ?? 'Well Used';
    $size = null;
    if ($category === 'Fashion') {
    ...
    if (isset($_FILES['images']) && !empty($_FILES['images']['name'][0])) {
        foreach ($_FILES['images']['tmp_name'] as $key => $tmp_name) {
            if ($tmp_name && $_FILES['images']['error'][$key] === 0) {
                ...
                $filename = 'prod_' . time() . '_' . $key . '.' . strtolower($ext);
    ...
    try {
        $sql = "INSERT INTO products 
                (name, price, size, `condition`, category, subcategory, image1, image2, image3, image4, image5, description)
                VALUES 
                (:name, :price, :size, :condition, :category, :subcategory, :image1, :image2, :image3, :image4, :image5, :description)";
...
<link rel="stylesheet" href="add-product-style.css?v=<?= time() ?>">
            ...
            <div class="form-grid">
                <div class="form-left">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="price" required min="10000" step="1000">
                    </div>
                </div>

                <div class="form-right">
                    <div class="form-group">
                        <label>Product Image (maks 5, wajib 1)</label>
